...
5. **[[duckdb-layer]] / reports** — trial balance always ties; BS==cumulative; On any fixture ledger the accrual-vs-cash delta must reconcile **line by line to an enumerated set** — unpaid documents (R4), retainers recognized at receipt (R6), non-cash settlements such as a bad-debt write-off (R5), prepaid timing (R12) — **with an unattributed cent failing the gate, and cost-recovery entries (R11) contributing exactly 0.00.** Both prior wordings were wrong: "unpaid/non-cash items" licensed the R11 cost-recovery bug; "unpaid documents only" fails a correct ledger by the retainer and the write-off. **(proposed 2026-07-31)**; corrupted sheet value → hard parse error (the alarm test).
