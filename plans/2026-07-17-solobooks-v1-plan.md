...
- [ ] `AccountType` (ASSET/LIABILITY/EQUITY/INCOME/EXPENSE), `DetailType` — **enumerate in full, no ellipsis: it is the RECOGNITION KEY that R1–R12 branch on, not report grouping** (2026-07-31 second-pass W1/W6): BANK, CASH, CREDIT_CARD, MERCHANT_CLEARING (these four = `MONEY_ACCOUNT`), DEPRECIATION, AMORTIZATION, DEPLETION (= `CostRecovery`), PREPAID_ASSET, ACCUMULATED_DEPRECIATION, ACCUMULATED_AMORTIZATION, AR, AP, FIXED_ASSET, UNEARNED_REVENUE, SALES_TAX_PAYABLE, EQUITY_OBE, **GAIN_ON_DISPOSAL, LOSS_ON_DISPOSAL**, and residuals **OTHER_INCOME / OTHER_EXPENSE / OTHER_ASSET / OTHER_LIABILITY / OTHER_EQUITY** so no rule falls back to matching an account name. Lint: every seed account maps to a member. **`v_pnl_cash` must never match on account name or number**, `DocType` (INVOICE, PAYMENT, BILL, BILL_PAYMENT, SALES_RECEIPT, CREDIT_NOTE, VENDOR_CREDIT, TRANSFER, JOURNAL_ENTRY), `DocStatus` (DRAFT/POSTED/VOIDED), `GLColumn` (ordered — TXN_ID, DATE, TYPE, REF, **REVERSES_TXN_ID**, ACCOUNT_NUMBER, ACCOUNT_NAME, DEBIT, CREDIT, CUSTOMER, PROJECT, VENDOR, CLASS, LOCATION, DUE_DATE, **LINE_REASON**, MEMO, POSTED_AT, POSTED_BY). Test: GLColumn order matches wiki `general-ledger-sheet`.
- [ ] **Process-aware vocabulary (2026-07-30 realignment — these freeze here, add them now):** `ProcessType` (IMPORT_RUN, REC_RUN, CLOSE_RUN, AMENDMENT, ONBOARDING), `TriggerKind` (USER_CHAT, SCHEDULE, IMPORT, CLOSE, SYSTEM), `ProcessState` (OPEN, COMPLETED, ABANDONED), `DecisionKind` (MATCH, APPROVAL, REJECTION, CATEGORIZATION, TOLERANCE_WRITE_OFF, DUPLICATE_DISPOSITION, UNCATEGORIZED_PARKING, CLEARING, LOCK, UNLOCK, VOID, AMENDMENT), `ActorKind` (HUMAN, AGENT, RULE, SYSTEM), `Confidence` (EXACT, HIGH, MEDIUM, LOW, AMBIGUOUS), `ExceptionKind` (UNCATEGORIZED, UNAPPLIED_CREDIT, STALE_UNEARNED, NONZERO_OBE, RECONCILIATION_DISCREPANCY, PENDING_DRAFT, TRIAL_BALANCE_BREAK, DUPLICATE_SUSPECTED, IMPORT_ROW_REJECTED, PREVIEW_DRIFT, OUTSTANDING_ITEM_AGED, EVIDENCE_MISSING, REC_PERIOD_DISCONTINUITY, CUTOFF_DATE_UNCONFIRMED, **OFFSET_INTENT_UNDECLARED, WRITE_OFF_INTENT_UNDECLARED, PREPAYMENT_PERIOD_UNCONFIRMED, PREPAID_AMORTIZATION_UNLINKED, UNMAPPED_RECOGNIZED_LINE, DISPOSAL_FORM_4797, OPENING_JE_OVERLAP_BREAK, HISTORICAL_DOC_POSTS_OBE**), `PriorReturnBasis` (CASH, ACCRUAL — no NONE: absent an accrual return the answer is CASH), **`DecisionKind` += PRIOR_RETURN_BASIS, MIGRATION_COMPLETE, CUTOFF_DATE_CONFIRMATION, STATEMENT_ROW_DISPOSITION, PREPAYMENT_12_MONTH_TEST — without the last, R12's entire carrier has no legal record to write and every prepayment silently defaults to capitalized**; `DocType` += REVERSAL; **delete `OriginType`** (declared with zero members and no reader), `Disposition` (OPEN, RESOLVED, ACKNOWLEDGED), `EvidenceKind` (DRIVE_FILE, STATEMENT_ROW, CONVERSATION, IMPORT_ROW, ARTIFACT), `OriginType`, `LineReason` (TOLERANCE_WRITE_OFF, ROUNDING, OPENING_BALANCE, NSF_REVERSAL, **COST_RECOVERY**, **OFFSET_SETTLEMENT**, **PREPAID_12_MONTH**, **PRINCIPAL_WRITE_OFF** — the last four are read as predicates by R11, R5a, R12 and R3 respectively; **freezes at the first posting**), `Side` (DEBIT, CREDIT), `PostingSource` (CHAT, CSV_IMPORT, RECURRING, RECONCILIATION), `ObjectLens` (SETTLEMENT, TREATMENT, DECISION, EVIDENCE, FULL). Wiki: `decision-record`, `process-instance`, `safety-nets`, `context-assembly`. Commit `feat: domain enums`.
...
- [ ] **Ruleset fixtures (R1–R12, 2026-07-30 tri-persona review + 2026-07-31 second pass) — these encode the rule changes and must exist before any posting:** `ar_ap_offset` **split into `_equal` / `_walkaway` / `_cross_party` / `_with_cash_leg`** — the bare name is specified twice with different contents and whichever YAML is written second silently wins (2026-08-01 W-12) (R5a: the offset leg recognizes on the JE date, the cash remainder when it settles — the fixture as previously scoped would have encoded the *wrong* answer); `prior_return_cash_migration` and `prior_return_accrual_migration` (R4 branch), plus `prior_return_cash_capitalized_bill` (**asserts `expect.balances` Machinery = $12,000 — not $24,000 and not $0**; the bill's own `gl_lines` are identical in all three cases, so a gl_lines-only assertion passes the very defect it exists to catch. A historical unpaid bill must compose `DR Machinery & Equipment`, NOT `DR Equipment Expense` — hardcoding the expense deducts it at settlement and again via §179/MACRS), and `depreciation_and_179` (R11 — the manual depreciation JE that v1-scope mandates must hit BOTH bases); `cutoff_december_check` (R10 — a cheque written 12/28 clearing 01/04 deducts in the earlier year, and the rec still ties); plus the second-pass set: `prepaid_12_month` and `prepaid_multi_year` (R12 — exactly one of payment-recognition and amortization-recognition fires, never both, never neither); `bundled_yearend_je` (R11 line-level — `DR Depreciation 500 / DR Bad Debt 1,200 / CR Accum Dep 500 / CR AR 1,200` recognizes **$500**, not $1,700); `ar_ap_offset_cross_party` (R5a intent predicate — an unrelated bad debt batched with an unrelated vendor credit in one JE recognizes **0.00**, identically to two JEs); `payment_with_principal_haircut` (R3/R9 — a $900 payment on a $1,000 invoice with a $100 write-off reports $900 of receipts and no §166 deduction); `owner_paid_capital_asset` (R1(c) — a $4,200 camera composes `DR Camera Equipment`, deducted once); `rec_reproducibility` (W12 — run December → run January → re-run December, still 0.00). **(2026-07-31)**
...
- [ ] `v_lines` (⋈ accounts, signed amount by normal balance) → `v_balances` → `v_open_items` (ref sums) → `v_pnl` → `v_pnl_cash`.
- [ ] **`v_pnl_cash` implements R1–R12 by QUOTING `cash-basis-recognition`, never paraphrasing it (2026-08-01 fourth-pass W-1).** This task has now reproduced a superseded ruleset in three consecutive passes; `views.py` is built from this file, so a paraphrase here silently outranks the wiki. Required, verbatim from the wiki: the predicate is per-**(`gl_line`, recognition event)** with **`Σ recognized ≤ line amount`** (never "once per line, on exactly one date" — that outlaws partial settlement); the **ladder table** exactly as published, including R11 above the R5/R5a suppression step, R6's rung, the **untyped** default, and R1/R4/R7/R9/R10 declared outside; **R11's two limbs** (`detail_type ∈ CostRecovery` **OR** `line_reason = COST_RECOVERY`); **R12's tag branches**, with *the view never evaluating the 12-month rule and having no benefit period to evaluate it from*; **`applied` signed**, so reversals and refunds reach.
- [ ] Grep-tests: `views.py` references no field outside `gl_lines` / `accounts` / `rec_clears`; **`views.py` contains no date arithmetic on a benefit period**; **no `prior_return_basis` branch** (Task 2.8 makes it unreachable by design — the answer is baked into the posted documents' composition). Tests on fixture ledgers incl. the cash-basis delta assertion. Commit `feat: canonical view stack`.
...
- [ ] `parse_statement(csv_bytes) -> list[StatementRow]` (**`txn_date`, `post_date`** — both nullable, mapping recorded on the `IMPORT_RUN`; R10 conditions on the discriminator and the old single `date` cannot represent it — description, amount, direction) tolerant of real bank variance — fixtures: Chase-shaped, BofA-shaped, generic debit/credit-column CSV; malformed rows collected into a reject report, never silently dropped. Commit `feat: statement parsing`.
