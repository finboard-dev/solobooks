...
- **Next.js UI (all of it):** login, "connect Claude" page (MCP URL + auth), folder view linking into the view-only Drive folder, and the **batch review grid** — the ONE sanctioned exception to "never renders a ledger": a read-only import-draft queue (date, description, proposed account, amount, dup-flag; approve/reject per row) wired to the [[approval-flow]] tools. It renders a draft queue, not the ledger ([[product-vision]]).
